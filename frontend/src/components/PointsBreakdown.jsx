...
const rules = [
  ...
  { label: 'Call Made', points: '2 pts', color: 'text-mvp-electric', bg: 'bg-mvp-electric/10' },
  ...
  return (
    <div className="bg-mvp-card rounded-xl border border-mvp-border p-5">
      <div className="flex items-center gap-2 mb-4">
        <Zap size={18} className="text-mvp-warning" />
        <h2 className="text-lg font-bold">Points System</h2>
      </div>
      ...
      <div className="space-y-2 mb-4">
        {rules.map((rule) => (
          <div key={rule.label} className="flex items-center justify-between p-2 rounded-lg bg-mvp-dark border border-mvp-border">
            ...
            <span className={`text-sm font-bold ${rule.color}`}>{rule.points}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-xs text-white/30 uppercase tracking-wider">Streak Multipliers</span>
      </div>
      <div className="space-y-2">
        {multipliers.map((m) => (
          <div key={m.label} className="flex items-center justify-between p-2 rounded-lg bg-mvp-dark border border-mvp-border">
            ...
            <span className={`text-sm font-bold ${m.color}`}>{m.mult}</span>
          </div>
        ))}
      </div>
    </div>
